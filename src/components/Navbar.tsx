...
const css = `
  ...
  /* ── Desktop nav links — white palette, hover = mauve ── */
  ...
  .nb-cta {
    ...
    box-shadow: 0 4px 20px rgba(109,40,217,.3), inset 0 1px 0 rgba(255,255,255,.12);
  ...
  .nb-cta:hover {
    box-shadow: 0 6px 28px rgba(109,40,217,.5), 0 0 0 1px rgba(167,139,250,.4), inset 0 1px 0 rgba(255,255,255,.15);
  ...
  .nb-drawer.nb-open { max-height: 800px; opacity: 1; }
  ...
  /* mobile links — white, hover = mauve */
  .nb-m-link {
    font-size: .78rem;
    letter-spacing: .07em;
    text-transform: uppercase;
  ...
  .nb-m-link:last-of-type { border-bottom: none; }
  .nb-m-link:hover { color: rgba(196,170,255,.85); }
  ...
  .nb-m-cta {
    ...
    transition: box-shadow .25s, transform .2s, border-color .3s;
  }
  .nb-m-cta:hover {
  ...
  return (
    <nav className={['nb-root', scrolled ? 'nb-scrolled' : ''].join(' ')}>
      ...
      <div className="nb-inner">
        <button className="nb-logo" onClick={handleLogoClick} aria-label="Go to home">
        ...
        <button className="nb-burger" onClick={() => setOpen(v => !v)} aria-label="Toggle menu">
      ...
      <div className={['nb-drawer', open ? 'nb-open' : ''].join(' ')}>
        {/* Glass when at top, solid when scrolled */}
        <div className={['nb-drawer-inner', scrolled ? 'nb-solid' : 'nb-glass'].join(' ')}>
          {links.map(l => (
            <a key={l.label} href={l.href} className="nb-m-link" onClick={() => setOpen(false)}>
              {l.label}
            </a>
          ))}
          <a href="#packages" className="nb-m-cta" onClick={() => setOpen(false)}>
            Get Started
          </a>
        </div>
      </div>
